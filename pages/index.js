...
export default function Landing() {
  ...
  const handleSubmit = async (e) => {
    ...
    else if (error.code === '23505') setSent(true);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-sky-500/30 flex flex-col">
      <Head>
        <title>3ADK Vault | Capture Haute Qualité</title>
        <script src="https://cdn.tailwindcss.com"></script>
      </Head>

      <nav className="max-w-7xl w-full mx-auto px-6 py-8 flex justify-between items-center text-sm">
        <div className="text-2xl font-bold bg-gradient-to-r from-sky-400 to-blue-600 bg-clip-text text-transparent italic tracking-tighter shrink-0 transition-opacity hover:opacity-80">
      ...
      <main className="flex-grow flex flex-col items-center justify-center max-w-4xl mx-auto px-6 text-center py-20">
        <div className="inline-block px-4 py-1.5 mb-8 text-[10px] font-bold tracking-[0.2em] text-sky-400 uppercase bg-sky-400/5 rounded-full border border-sky-400/20">
          Accès Privilégié - Note Technique 2026
        </div>
        ...
        {!sent ? (
          <form onSubmit={handleSubmit} className="w-full max-w-md bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl space-y-4">
            <input 
              ...
              required
              className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-6 py-4 focus:outline-none focus:border-sky-500 transition-all text-white placeholder-slate-600"
            ...
            <select 
               required
               className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-6 py-4 focus:outline-none focus:border-sky-500 transition-all text-slate-400"
            ...
            <button 
              disabled={loading}
              className="w-full bg-white text-slate-950 px-8 py-5 rounded-2xl font-black text-lg hover:bg-sky-400 hover:scale-[1.02] transition-all disabled:opacity-50 mt-4"
            >
              {loading ? 'Accréditation...' : 'Accéder au Livre Blanc'}
            </button>
            <p className="text-[10px] text-slate-600 uppercase tracking-widest mt-4">Traitement souverain des données • Art. 28 RGPD</p>
          </form>
        ) : (
          <div className="bg-sky-500/10 border border-sky-400/20 rounded-[3rem] p-12 max-w-md mx-auto shadow-2xl backdrop-blur-sm animate-in fade-in zoom-in duration-500">
